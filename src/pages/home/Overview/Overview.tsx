...
const events = [
  { id: 1, title: "worshipRehearsal", date: "2024-07-10", place: "mainHall" },
  { id: 2, title: "prayerMeeting", date: "2024-07-12", place: "smallHall" },
];

const news = [
  { id: 1, title: "childrenCamp", date: "2024-06-20" },
  { id: 2, title: "renovation", date: "2024-06-15" },
];

const Overview = () => {
  const { t } = useTranslation("pages/overview");
  const [notifications, setNotifications] = useState([{ id: 1, ministry: "prayer" }]);
    ...
    <div className="p-4 max-w-5xl mx-auto">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-blue-900 font-medium">
                  {t("notifications.invited")} "{t(`ministries.names.${notif.ministry}`)}"
                </span>
              </div>
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-8">
        <div className="flex-1 flex flex-col items-center md:items-start bg-white rounded-xl shadow p-6 border border-gray-100">
          ...
          <span className="inline-block mb-2 text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded font-semibold">
            {t(`ministries.names.${userProfile.role}`)}
          </span>
          <p className="mt-2 text-gray-700 text-sm text-center md:text-left">{t(`profile.${userProfile.about}`)}</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow p-6 border border-gray-100 mb-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="font-semibold text-gray-800 text-lg">{t("events.title")}</span>
        </div>
        <ul className="divide-y divide-gray-100">
          {events.map((event) => (
            ...
            >
              <div>
                <span className="font-medium text-gray-700">{t(`events.items.${event.title}`)}</span>
                <span className="ml-2 text-xs text-gray-400">{t(`events.places.${event.place}`)}</span>
              </div>
              <span className="text-xs text-gray-500 mt-1 sm:mt-0">{event.date}</span>
            </li>
          ))}
          {events.length === 0 && <li className="text-gray-400">{t("events.empty")}</li>}
        </ul>
      </div>
      ...
      <div className="bg-white rounded-xl shadow p-6 border border-gray-100 mb-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="font-semibold text-gray-800 text-lg">{t("friends.title")}</span>
        </div>
        <div className="flex gap-4 flex-wrap">
          ...
          {friends.length === 0 && <div className="text-gray-400">{t("friends.empty")}</div>}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow p-6 border border-gray-100">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="font-semibold text-gray-800 text-lg">{t("news.title")}</span>
        </div>
        <ul className="divide-y divide-gray-100">
          {news.map((item) => (
            <li key={item.id} className="py-2 flex items-center justify-between">
              <span className="text-gray-700 font-medium">{t(`news.items.${item.title}`)}</span>
              <span className="text-xs text-gray-500">{item.date}</span>
            </li>
          ))}
          {news.length === 0 && <li className="text-gray-400">{t("news.empty")}</li>}
        </ul>
      </div>
    </div>
  );
